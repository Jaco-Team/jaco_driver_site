...
export const useOrdersStore = createWithEqualityFn(
  (set, get) => ({
    orders: [],
    isOpenMenu: false,
    update_interval: 30,
      ...
      try {
        ...
        if (Array.isArray(json?.orders)) {
          ...
          set({
            ...
            del_orders: Array.isArray(json?.arr_del_list)
              ? json.arr_del_list.map(normalizeOrderRow)
              : [],
          ...
          if (
            !get().home &&
            json?.home?.latitude !== undefined &&
            json?.home?.longitude !== undefined
          ) {
            set({
              // home: json?.home,
              home: {
    ...
    getSettings: async (token) => {
      ...
      set({
        is_need_avg_time: hasField('driver_avg_time')
          ? normalizeBoolLike(settings.driver_avg_time)
          : currentState.is_need_avg_time,
        is_need_page_stat: hasField('driver_page_stat_time')
          ? normalizeBoolLike(settings.driver_page_stat_time)
...
export const usePriceStore = createWithEqualityFn(
  (set, get) => ({
    statPrice: null,
    give_hist: [],
...
export const useGraphStore = createWithEqualityFn(
  (set, get) => ({
    ...
    getGraph: async (date, token) => {
      ...
      set({
        month_list: Array.isArray(json?.month) ? json.month : [],
