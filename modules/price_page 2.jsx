...
export default function PricePage() {
  ...
  const [statPrice, give_hist, getStat, getStatBetween] = usePriceStore((state) => [
    state.statPrice,
    state.give_hist,
    state.getStat,
    ...
    <Meta title="Расчет">
      <Grid container spacing={3} className={'price ' + roboto.variable}>
        ...
        <Grid item xs={12} style={{ textAlign: 'center' }}>
          <h1 style={{ margin: 0, fontSize: 48, textAlign: 'center' }}>
            {new Intl.NumberFormat('ru-RU').format(statPrice?.my_price)} ₽
          </h1>
          <TextDescription
            text="Сумма налички"
            ...
            <Table>
              ...
              <TableBody>
                {give_hist.map((rowData, index) => (
                  <TableRow hover key={index}>
                    ...
                    <TableCell style={{ fontSize: globalFontSize }}>
                      {new Intl.NumberFormat('ru-RU').format(rowData.give)} ₽
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
              ...
              <TableFooter>
                <TableRow>
                  ...
                  <TableCell style={{ fontSize: globalFontSize }}>
                    {new Intl.NumberFormat('ru-RU').format(statPrice?.full_give)} ₽
                  </TableCell>
                </TableRow>
                <TableRow>
                  <TableCell style={{ fontSize: globalFontSize }}>Осталось сдать</TableCell>
                  <TableCell style={{ fontSize: globalFontSize }}>
                    {new Intl.NumberFormat('ru-RU').format(statPrice?.my_cash)} ₽
                  </TableCell>
                </TableRow>
              </TableFooter>
            </Table>
