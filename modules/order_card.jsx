...
export default function OrderCard({ item, is_map = false, globalFontSize }) {
  ...
  const [actionGetOrder, actionPayOrder, setActiveConfirm, driver_pay] = useOrdersStore((state) => [
    state.actionGetOrder,
    state.actionPayOrder,
  ...
  return (
    <div
      className={'OrderCard ' + roboto.variable}
      style={{ backgroundColor: parseInt(item.is_delete) === 1 ? '#d95030' : '#fff' }}
    >
      ...
      <div className="counts">
        {parseInt(item.count_other) == 0 ? null : (
          <Typography
            className="Blue"
            style={{ marginRight: 5, fontSize: globalFontSize }}
            component="span"
          >
            Роллы
          </Typography>
        ...
        {parseInt(item.count_pizza) == 0 ? null : (
          <Typography
            className="Red"
            style={{ marginLeft: 5, marginRight: 5, fontSize: globalFontSize }}
            component="span"
          >
            Пицца x{item.count_pizza}
          </Typography>
        )}
      ...
      <div className="addr">
        ...
        <div className="addr_2">
          {item.pd.length == 0 || parseInt(item.pd) == 0 ? null : (
            <>
              <Typography component="b" style={{ fontSize: globalFontSize }}>
                Пд:
              </Typography>
              <Typography component="span" style={{ fontSize: globalFontSize }}>
                {item.pd},{' '}
        ...
        {parseInt(item.fake_dom) == 0 ? (
          <Typography className="addr_3" component="span" style={{ fontSize: globalFontSize }}>
            Домофон не работает
          </Typography>
        ) : null}
      ...
      <div className="block_price">
        ...
        {parseInt(item.online_pay) == 1 ? (
          <Typography component="span" className="is_paid" style={{ fontSize: globalFontSize }}>
            Оплачено
          </Typography>
        ) : (
          <Typography component="span" style={{ fontSize: globalFontSize }}>
      ...
      {parseInt(item.is_get) == 0 ? (
        <div className="action_get">
          <a href={'tel:' + item.number} style={{ fontSize: globalFontSize }}>
            {item.number}
          </a>
          <Button
            onClick={() => actionGetOrder(item.id, is_map)}
        ...
        <div className="action_my">
          <div>
            {parseInt(item.status_order) == 6 ? null : (
              <Button
                onClick={() => setActiveConfirm(true, item.id, is_map, 'cancel', item.is_delete)}
                style={{ fontSize: globalFontSize }}
              >
                Отменить
              </Button>
          ...
          {parseInt(item.status_order) == 6 ? null : parseInt(item.is_my) === 1 &&
            parseInt(item.online_pay) === 0 &&
            parseInt(driver_pay) == 1 ? (
            <Grid className="finish_group">
              <Button
                onClick={() => setActiveConfirm(true, item.id, is_map, 'finish', item.is_delete)}
                style={{ fontSize: globalFontSize }}
              >
                Завершить
              </Button>
              <Button
                onClick={() => actionPayOrder(item.id, is_map)}
                style={{ fontSize: globalFontSize }}
              >
                <QrCodeScannerIcon />
              </Button>
          ...
          {parseInt(item.status_order) == 6 ? null : (
            <Button
              className="client"
              onClick={() => setActiveConfirm(true, item.id, is_map, 'fake', item.is_delete)}
              style={{ fontSize: globalFontSize }}
            >
        ...
        <div className="action_other">
          ...
          <a href={'tel:' + item.driver_login} style={{ fontSize: globalFontSize }}>
            {item.driver_login}
          </a>
        </div>
      )}
