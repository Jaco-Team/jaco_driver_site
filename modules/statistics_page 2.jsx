...
export default function StatisticsPage() {
  const [initialRange] = useState(() => {
    const range = normalizeRangeWithReasons(
    ...
    return {
      ...
      message: reasonsToMessage(range.reasons, range.s, range.e, range.minDate),
    };
  });
  const [date_start, setDateStart] = useState(initialRange.start);
  const [date_end, setDateEnd] = useState(initialRange.end);
  const initialMessageRef = useRef(initialRange.message);
  ...
  useEffect(() => {
    ...
    if (msg) {
      ...
      setTimeout(() => showSnackbar(msg), 0);
    }
    getStatistics(token, fmt(initialRange.start), fmt(initialRange.end));
  }, [getStatistics, initialRange.end, initialRange.start, token]);
