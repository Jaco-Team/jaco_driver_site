...
type StatisticsSnackbar = SnackbarOrigin & {
  open: boolean;
  message: string;
};
...
export default function StatisticsPage() {
  ...
  const [snackbar, setSnackbar] = useState<StatisticsSnackbar>({
    open: false,
    vertical: 'top',
    horizontal: 'center',
    message: '',
  ...
  useEffect(() => {
    ...
    getStatistics(fmt(initialStartDate), fmt(initialEndDate));
  }, [getStatistics, initialEndDate, initialStartDate, session?.isAuth]);

  const openPicker = (type: Exclude<ActivePicker, null>) => {
  ...
  const applyDraftDate = () => {
    ...
    if (session?.isAuth === true) {
      getStatistics(fmt(normalizedRange.s), fmt(normalizedRange.e));
    }
